...
This folder holds **only what you need to run** the pipeline locally (code + templates). **UI, CLI messages, and source comments are in English.** The **Project report** subsection below keeps **three narrative languages** (English, Portuguese, Turkish) on purpose. Quick start is at the **bottom**.
...
**Layer 1 (Profiling)** interprets PDF content with a custom heuristic regex classifier. It pulls relevant terms from a pool of 17 mathematical keywords, assigns the problem to one of four main categories (scalar surface integral, flux integral, divergence theorem, Stokes theorem), and detects surface type (sphere, paraboloid, cylinder, cone, plane, hemisphere, torus). A typical PDF yields 5–7 keywords in about 0.002 s. In phase 6 this layer was extended with ten more categories (improper integral, definite integral, double/triple integral, derivative, limit, series, differential equation, linear algebra, equation, etc.), so the system can follow two solution strategies: `surface_integral` or `general_math`.
...
Prompt design matters: the VLM is instructed to **read only, not solve**—an eight-rule system prompt blocks solution steps, forbids `\boxed{}` answers, and stresses reading every symbol carefully. **Dual-pass** calls per page pick the better output by quality score and length. `clean_output()` strips lines that look like reasoning (“we get”, “substituting”, “therefore”) while keeping the problem statement.
...
**Layer 6 (Verification)** checks the LLM answer with **SymPy**: numeric comparison (tolerance 0.01, 48 problems), symbolic comparison (parametric answers, 5 problems), and string search as fallback. The LaTeX parser that pulls `\boxed{}` (and similar) into SymPy went through four major versions—v1 only handled `\frac`/`\sqrt` (40/58 parse success); **v4** adds powers, parametric symbols, spacing-based implicit multiplication, and guarded functions and reached **58/58** parse success.
